...
class NewMap extends React.Component {
    constructor(props) {
        ...
        this.state = {
            zoom: 12,
            start: [],
            center: [this.props.user.location ? this.props.user.location : "San Francisco"],
            waypoints: [],
            destination: [],
            polyline: "",
            test: "",
    ...
    componentDidMount() {
        ...
        if (this.state.address) {
            axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/${address}.json?access_token=pk.eyJ1IjoicHJvc2UwMDIxIiwiYSI6ImNrMzZoYWdidTAxcm8zaW82MW5jZmV6c2EifQ.PRbSpg500wqcoctnYFTIog&cachebuster=1574441136340&autocomplete=true&types=place&limit=1`)
                .then(res => {
                    mapboxgl.accessToken = "pk.eyJ1IjoicHJvc2UwMDIxIiwiYSI6ImNrMzZoYWdidTAxcm8zaW82MW5jZmV6c2EifQ.PRbSpg500wqcoctnYFTIog"
                    const map = new mapboxgl.Map({
                        container: this.mapContainer,
